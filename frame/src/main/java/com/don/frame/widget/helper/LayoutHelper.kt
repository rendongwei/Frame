...
class LayoutHelper : ILayout {
    ...
    constructor(context: Context?, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int, owner: View) {
        ...
        if (attrs != null || defStyleAttr != 0 || defStyleRes != 0) {
            var typedArray = context!!.obtainStyledAttributes(attrs, R.styleable.Layout, defStyleAttr, defStyleRes)
                ...
                outlineProvider = object : ViewOutlineProvider() {
                    override fun getOutline(v: View?, outline: Outline?) {
                        ...
                        if (mIsOutlineExcludePadding) {
                            left += paddingLeft
                            top += paddingTop
                            right = Math.max(left + 1, right - paddingRight)
                            bottom = Math.max(top + 1, bottom - paddingBottom)
                        }
    ...
    fun drawDividers(canvas: Canvas, w: Int, h: Int) {
        mOwner?.get()?.apply {
            ...
            if (mBottomDividerHeight > 0) {
                ...
                var y = mBottomDividerHeight / 2.0f
                canvas.drawLine(
